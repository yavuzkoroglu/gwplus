    ...
    do
        ...
        then
            ...
            then
                ...
                echo "${minlen_edgepair_50},${x}" >> rq2_raw.csv
            elif [[ $ratio -eq 60 ]]
            then
                x=$(jq -n ${len}/${minlen_edgepair_60})
                echo "${minlen_edgepair_60},${x}" >> rq2_raw.csv
            elif [[ $ratio -eq 70 ]]
            then
                x=$(jq -n ${len}/${minlen_edgepair_70})
                echo "${minlen_edgepair_70},${x}" >> rq2_raw.csv
            elif [[ $ratio -eq 80 ]]
            then
                x=$(jq -n ${len}/${minlen_edgepair_80})
                echo "${minlen_edgepair_80},${x}" >> rq2_raw.csv
            elif [[ $ratio -eq 90 ]]
            then
                x=$(jq -n ${len}/${minlen_edgepair_90})
                echo "${minlen_edgepair_90},${x}" >> rq2_raw.csv
            elif [[ $ratio -eq 100 ]]
            then
                x=$(jq -n ${len}/${minlen_edgepair_100})
                echo "${minlen_edgepair_100},${x}" >> rq2_raw.csv
            fi
        fi
    done < <(tail -n +2 rq12_${model}.csv)
done
