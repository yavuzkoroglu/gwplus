...
void free_patha(PathArray* const pathArray) {
    ...
    for (
        Path* const path = pathArray->array + pathArray->cap - 1;
...
void primePathsFromGWModel_patha(PathArray* const primePaths, GWModel const* const gwm) {
    ...
    for (uint32_t i = 0; i < pathStack->size; i++) {
        DEBUG_ASSERT(extend_path(pathStack->array[pathStack->size - i - 1], i, 1) == PATH_EXTEND_OK)
        NDEBUG_EXECUTE(extend_path(pathStack->array[pathStack->size - i - 1], i, 1))
    }
    ...
    while (pathStack->size > 0) {
        ...
        for (uint32_t i = size_outEdges - 1; i != 0xFFFFFFFF; i--) {
            ...
            clone_path(path_to_extend, path_at_hand, 1);
        ...
        if (potential_extensions_count == 0) {
            if (!IS_PRIME_PATH(path_at_hand)) {
                for (
                    Path* path = primePaths->array + primePaths->size - 1;
                    path >= primePaths->array;
                    path--
                ) {
                    if (!isValid_path(path) || IS_PRIME_PATH(path)) continue;

                    if (subpath_path(path_at_hand, path))
                        goto PATH_AT_HAND_IS_SUBSUMED__FORGET_IT;

                    if (subpath_path(path, path_at_hand))
            ...
            uint32_t i = 0;
            while (i < primePaths->size && isValid_path(primePaths->array[i])) i++;
            if (i == primePaths->size) {
                RECALLOC_IF_NECESSARY(Path, primePaths->array, uint32_t, primePaths->cap, primePaths->size, {RECALLOC_ERROR;})
                primePaths->size++;
            }
            clone_path(primePaths->array[i], path_at_hand);
