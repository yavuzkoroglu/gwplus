    ...
    do
        ...
        do
            ...
            do
                for seed in $GW_SEEDS
                do
                    java -jar graphwalker-cli-4.3.2.jar offline -d $seed -m "$EXPS/$model/m.json" "$method(${coverage}($ratio))" > $EXPS/$model/tests/graphwalker_$method_$coverage_$ratio_$seed.txt
                done
            done
        done
    done
